...
<body>
  ...
  const data = []
  for (let i = 1; i <= TOTAL_QUESTIONS; i++) { data.push({ id: i, content: `Đây là nội dung câu hỏi số <b>${i}</b>. Hãy
    chọn phương án đúng nhất trong các lựa chọn dưới đây? <br> (Nội dung demo để test giao diện hiển thị).`,
    ...
    <label class="option-item">
      <input type="radio" name="currentQuestion" class="option-radio form-check-input" value="${i}" ${savedAns===i
        ? "checked" : "" } onchange="selectAnswer(${q.id}, ${i})">
    ...
    if (newIdx >= 0 && newIdx < examData.length) { renderQuestion(newIdx) } } // Nhảy đến câu bất kỳ từ bảng sheet
      function goToQuestion(idx) { renderQuestion(idx) } // Đánh dấu (Flag) function toggleFlag() { const
      qId=examData[currentIdx].id if (flaggedSet.has(qId)) { flaggedSet.delete(qId) } else { flaggedSet.add(qId) }
      updateFlagButtonUI() updateSheetFlagUI(qId) } function updateFlagButtonUI() { const qId=examData[currentIdx].id if
      (flaggedSet.has(qId)) { els.btnFlag.classList.remove("btn-outline-warning")
      els.btnFlag.classList.add("btn-warning") els.btnFlag.innerHTML='<i class="bi bi-flag-fill"></i> Đã đánh dấu' }
      else { els.btnFlag.classList.add("btn-outline-warning") els.btnFlag.classList.remove("btn-warning")
      els.btnFlag.innerHTML='<i class="bi bi-flag"></i> Đánh dấu' } } function updateSheetFlagUI(qId) { // Tìm dòng
      tương ứng trong sheet để đổi màu hoặc thêm icon (Demo đổi màu số câu) const
      label=document.getElementById(`q-label-${currentIdx}`) // Ở đây đơn giản ta cho ô số câu hỏi màu vàng if
      (flaggedSet.has(qId)) { label.style.backgroundColor="#ffc107" } else { label.style.backgroundColor="" // Reset //
      Add lại class active nếu đang active if (currentIdx===qId - 1) label.classList.add("active") } }
      /*=========================================6. TIMER VÀ SUBMIT=========================================*/ function
      startTimer() { timerInterval=setInterval(()=> {
      if (timeLeft <= 0) { clearInterval(timerInterval) alert("Hết giờ làm bài!") return } timeLeft-- const
        m=Math.floor(timeLeft / 60) .toString() .padStart(2, "0" ) const s=(timeLeft % 60).toString().padStart(2, "0" )
        els.timer.innerText=`${m}:${s}` }, 1000) } function confirmSubmit() { const
        doneCount=Object.keys(userAnswers).length document.getElementById("modal-done").innerText=doneCount
        document.getElementById("modal-remain").innerText=TOTAL_QUESTIONS - doneCount const myModal=new
        bootstrap.Modal(document.getElementById("submitModal")) myModal.show() }
        /*=========================================7. MAIN RUN=========================================*/ // Chạy khi
        load trang initSheet() // Vẽ bảng 40 câu renderQuestion(0) // Vào câu 1 startTimer() // Đếm giờ </script> -->
</body>
