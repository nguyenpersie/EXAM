        ...
        <div class="card shadow">
          <div class="card-header bg-primary text-white">
            <div class="d-flex justify-content-between align-items-center">
              ...
              <a href="{{ route('exams.show', $question->exam_id) }}" class="btn btn-light btn-sm">
            ...
            <form action="{{ route('questions.update', $question->id) }}" method="POST" enctype="multipart/form-data">
                ...
                @foreach(['A', 'B', 'C', 'D'] as $index => $letter)
                  ...
                  <div class="card mb-3 border-{{ $option && $option->is_correct ? 'success' : 'secondary' }}">
                    <div class="card-header bg-{{ $option && $option->is_correct ? 'success' : 'light' }} bg-opacity-10">
                      <div class="form-check">
                        <input type="radio" class="form-check-input" name="correct_answer" value="{{ $index }}"
                          id="correct_{{ $index }}" {{ $option && $option->is_correct ? 'checked' : '' }} required>
                    ...
                    <div class="card-body">
                      ...
                      <input type="file" class="form-control" name="options[{{ $index }}][image]" accept="image/*"
                        onchange="previewOptionImage(this, {{ $index }})">
                      <small class="text-muted">Hình ảnh cho đáp án {{ $letter }} (tùy chọn)</small>

                      @if($option && $option->image)
                        <div class="mt-2">
                          <img src="{{ asset('storage/' . $option->image) }}" alt="Hình đáp án {{ $letter }}"
                            class="img-thumbnail" style="max-height: 150px;">
                          <div class="form-check mt-1">
                            <input type="checkbox" class="form-check-input" name="options[{{ $index }}][remove_image]"
              ...
              <div class="row">
                ...
                <div class="col-md-4 mb-3">
                  ...
                  <input type="text" class="form-control" name="section" value="{{ old('section', $question->section) }}"
                ...
                <div class="col-md-4 mb-3">
                  ...
                  <input type="text" class="form-control" name="category"
                    value="{{ old('category', $question->category) }}" placeholder="VD: Biển báo">
                </div>
              </div>
